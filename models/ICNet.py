...
def build_icnet(inputs, label_size, num_classes, preset_model='ICNet', pooling_type = "MAX",
    ...
    if frontend == 'Res50':
        with slim.arg_scope(resnet_v2.resnet_arg_scope(weight_decay=weight_decay)):
            ...
            init_fn = slim.assign_from_checkpoint_fn(os.path.join(pretrained_dir, 'resnet_v2_18.ckpt'), slim.get_model_variables('resnet_v2_18'))
    elif frontend == 'Res18':
        with slim.arg_scope(resnet_v2.resnet_arg_scope(weight_decay=weight_decay)):
            logits_32, end_points_32 = resnet_v2.resnet_v2_18(inputs_4, is_training=is_training, scope='resnet_v2_18')
            logits_16, end_points_16 = resnet_v2.resnet_v2_18(inputs_2, is_training=is_training, scope='resnet_v2_18')
            logits_8, end_points_8 = resnet_v2.resnet_v2_18(inputs_1, is_training=is_training, scope='resnet_v2_18')
            resnet_scope='resnet_v2_18'
            # ICNet requires pre-trained ResNet weights
            init_fn = slim.assign_from_checkpoint_fn(os.path.join(pretrained_dir, 'resnet_v2_18.ckpt'), slim.get_model_variables('resnet_v2_18'))
